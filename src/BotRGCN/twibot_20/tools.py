...
def m_visualize_graph(
    ...
    if backend is None:
        backend = 'graphviz' if m_has_graphviz() else 'networkx'

    if backend.lower() == 'networkx':
        return _m_visualize_graph_via_networkx(edge_index, edge_weight, path, labels, predictions)
    elif backend.lower() == 'graphviz':
        return _m_visualize_graph_via_graphviz(edge_index, edge_weight, path, labels, predictions)

    raise ValueError(f"Expected graph drawing backend to be in "
                     f"{BACKENDS} (got '{backend}')")


def _m_visualize_graph_via_graphviz(
    edge_index: Tensor,
    edge_weight: Tensor,
    path: Optional[str] = None,
    labels: dict = None,
    ...
    for node in edge_index.view(-1).unique().tolist():
        color = "green" if predictions[node] == 0 else "red"
        g.node(str(labels[node]), color=color)
    ...
    for (src, dst), w in zip(edge_index.t().tolist(), edge_weight.tolist()):
        ...
        g.edge(str(labels[src]), str(labels[dst]), color=f'#{hex_color}{hex_color}{hex_color}')
...
def _m_visualize_graph_via_networkx(
    edge_index: Tensor,
    edge_weight: Tensor,
    path: Optional[str] = None,
    labels: dict = None,
    ...
    for node in edge_index.view(-1).unique().tolist():
        g.add_node(labels[node])

    for (src, dst), w in zip(edge_index.t().tolist(), edge_weight.tolist()):
        g.add_edge(labels[src], labels[dst], alpha=w)
